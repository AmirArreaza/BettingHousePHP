...
abstract class BaseEvento extends BaseObject  implements Persistent {
  ...
  const PEER = 'EventoPeer';
	...
	/**
	 * The value for the fkcategoria field.
	 * @var        int
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [fechaini] column value.
	 * 
	 *
	...
	public function getFechaini($format = 'Y-m-d')
	{
		...
		if ($this->fechaini === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->fechaini);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->fechaini, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [horaini] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getHoraini($format = 'H:i:s')
	{
		...
		try {
			$dt = new DateTime($this->horaini);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->horaini, true), $x);
		}
	...
	/**
	 * Get the [fkcategoria] column value.
	 * 
	 * @return     int
	 */
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = EventoPeer::NOMBRE;
		}
	...
	/**
	 * Sets the value of [fechaini] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setFechaini($v)
	{
		...
		if ( $this->fechaini !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->fechaini !== null && $tmpDt = new DateTime($this->fechaini)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->fechaini = ($dt ? $dt->format('Y-m-d') : null);
				$this->modifiedColumns[] = EventoPeer::FECHAINI;
			}
	...
	/**
	 * Sets the value of [horaini] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Evento The current object (for fluent API support)
	 */
	public function setHoraini($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->horaini !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->horaini !== null && $tmpDt = new DateTime($this->horaini)) ? $tmpDt->format('H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->horaini = ($dt ? $dt->format('H:i:s') : null);
				$this->modifiedColumns[] = EventoPeer::HORAINI;
			}
		} // if either are not null
	...
	public function setResultado($v)
	{
		...
		if ($this->resultado !== $v) {
			$this->resultado = $v;
			$this->modifiedColumns[] = EventoPeer::RESULTADO;
		}
	...
	/**
	 * Set the value of [fkcategoria] column.
	 * 
	 * @param      int $v new value
	 * @return     Evento The current object (for fluent API support)
	 */
	public function setFkcategoria($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->fkcategoria !== $v) {
			$this->fkcategoria = $v;
			$this->modifiedColumns[] = EventoPeer::FKCATEGORIA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->fechaini = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->horaini = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resultado = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->fkcategoria = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = EventoPeer::NUM_COLUMNS - EventoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCategoria !== null && $this->fkcategoria !== $this->aCategoria->getId()) {
			$this->aCategoria = null;
		}
	} // ensureConsistency
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 2:
				...
				break;
			case 3:
				return $this->getHoraini();
				break;
			case 4:
				return $this->getResultado();
				break;
			case 5:
				return $this->getFkcategoria();
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getHoraini(),
			$keys[4] => $this->getResultado(),
			$keys[5] => $this->getFkcategoria(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 2:
				...
				break;
			case 3:
				$this->setHoraini($value);
				break;
			case 4:
				$this->setResultado($value);
				break;
			case 5:
				$this->setFkcategoria($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setHoraini($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setResultado($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setFkcategoria($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EventoPeer::NOMBRE)) $criteria->add(EventoPeer::NOMBRE, $this->nombre);
		if ($this->isColumnModified(EventoPeer::FECHAINI)) $criteria->add(EventoPeer::FECHAINI, $this->fechaini);
		if ($this->isColumnModified(EventoPeer::HORAINI)) $criteria->add(EventoPeer::HORAINI, $this->horaini);
		if ($this->isColumnModified(EventoPeer::RESULTADO)) $criteria->add(EventoPeer::RESULTADO, $this->resultado);
		if ($this->isColumnModified(EventoPeer::FKCATEGORIA)) $criteria->add(EventoPeer::FKCATEGORIA, $this->fkcategoria);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setHoraini($this->horaini);

		$copyObj->setResultado($this->resultado);

		$copyObj->setFkcategoria($this->fkcategoria);
	...
	{
		if ($v === null) {
			$this->setFkcategoria(NULL);
		} else {
			$this->setFkcategoria($v->getId());
		}
	...
	{
		if ($this->aCategoria === null && ($this->fkcategoria !== null)) {
			$c = new Criteria(CategoriaPeer::DATABASE_NAME);
			$c->add(CategoriaPeer::ID, $this->fkcategoria);
			...
			/* The following can be used additionally to
			   ...
			   $this->aCategoria->addEventos($this);
			 */
		}
	 ...
	 * related Apuestas from storage. If this Evento is new, it will return
		...
		if ($this->collApuestas === null) {
			if ($this->isNew()) {
			   $this->collApuestas = array();
				...
				$criteria->add(ApuestaPeer::FKEVENTO, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Evento has previously been saved, it will retrieve
	 * related Equipoeventos from storage. If this Evento is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collEquipoeventos === null) {
			if ($this->isNew()) {
			   $this->collEquipoeventos = array();
				...
				$criteria->add(EquipoeventoPeer::FKEVENTO, $this->id);
	 ...
	 * actually need in Evento.
	 */
	public function getEquipoeventosJoinEquipo($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EventoPeer::DATABASE_NAME);
		}
		...
		if ($this->collEquipoeventos === null) {
			if ($this->isNew()) {
				...
				$this->collEquipoeventos = EquipoeventoPeer::doSelectJoinEquipo($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
